...
<div class="container-fluid" style="max-width: 900px;">
    <div class="mb-4 d-flex align-items-center">
        ...
        <h2 class="h4 mb-0">Editar Comunidad: {{ $comunidad->nombre }}</h2>
    </div>
        ...
        <div class="row g-4">
                ...
                <div class="card shadow-sm border-0 h-100">
                    <div class="card-header bg-white py-3 border-bottom-0">
                        <h5 class="card-title mb-0 fw-bold">
                            <i data-lucide="map-pin" class="me-2 text-primary"></i>Ubicación y Nombre
                        </h5>
                    </div>
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('parroquia_id') is-invalid @enderror" name="parroquia_id" id="parroquia_id" required>
                                ...
                                @foreach($parroquias as $parroquia)
                                    <option value="{{ $parroquia->parroquia_id }}" 
                                        {{ old('parroquia_id', $comunidad->parroquia_id) == $parroquia->parroquia_id ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="sector" name="sector" 
                                   placeholder="Ej. Sector Bajo" 
                                   value="{{ old('sector', $comunidad->sector) }}">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-5">
                <div class="card shadow-sm border-0 h-100">
                    <div class="card-header bg-white py-3 border-bottom-0">
                        <h5 class="card-title mb-0 fw-bold">
                            <i data-lucide="bar-chart-3" class="me-2 text-primary"></i>Demografía
                        </h5>
                    </div>
                    <div class="card-body">
                        <div class="row g-2">
                            <div class="col-6 mb-2">
                                ...
                                <input type="number" name="habitantes" class="form-control" 
                                       value="{{ old('habitantes', $comunidad->habitantes) }}">
                            </div>
                            <div class="col-6 mb-2">
                                <label class="small fw-bold">Familias</label>
                                <input type="number" name="familias" class="form-control" 
                                       value="{{ old('familias', $comunidad->familias) }}">
                            </div>
                            <div class="col-4">
                                <label class="small text-muted">Hombres</label>
                                <input type="number" name="hombres" class="form-control form-control-sm" 
                                       value="{{ old('hombres', $comunidad->hombres) }}">
                            </div>
                            <div class="col-4">
                                <label class="small text-muted">Mujeres</label>
                                <input type="number" name="mujeres" class="form-control form-control-sm" 
                                       value="{{ old('mujeres', $comunidad->mujeres) }}">
                            </div>
                            <div class="col-4">
                                <label class="small text-muted">Niños</label>
                                <input type="number" name="ninos" class="form-control form-control-sm" 
                                       value="{{ old('ninos', $comunidad->ninos) }}">
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="row">
                            <div class="col-md-3">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" name="usa_cisterna" id="usa_cisterna" 
                                        {{ old('usa_cisterna', $comunidad->usa_cisterna) ? 'checked' : '' }}>
                                    <label class="form-check-label" for="usa_cisterna">Usa Cisterna</label>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" name="agua_potable" id="agua_potable" 
                                        {{ old('agua_potable', $comunidad->agua_potable) ? 'checked' : '' }}>
                                    <label class="form-check-label" for="agua_potable">Agua Potable</label>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" name="zonas_silencio" id="zonas_silencio" 
                                        {{ old('zonas_silencio', $comunidad->zonas_silencio) ? 'checked' : '' }}>
                                    <label class="form-check-label" for="zonas_silencio">Zonas Silencio</label>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" name="tanques_grandes" id="tanques_grandes" 
                                        {{ old('tanques_grandes', $comunidad->tanques_grandes) ? 'checked' : '' }}>
